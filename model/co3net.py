...
class CompetitiveBlock(nn.Module):
    """
    DESCRIPTION: an implementation of the Competitive Block::   实现了竞争块（Competitive Block）的类
    ...
    def __init__(self, channel_in, n_competitor, ksize, stride, padding, init_ratio=1, o1=32, o2=12):
        ...
        self.gabor_conv2d = GaborConv2d(channel_in=channel_in, channel_out=n_competitor, kernel_size=ksize,
                                            stride=stride, padding=ksize // 2, init_ratio=init_ratio)
        self.gabor_conv2d2 = GaborConv2d(channel_in=n_competitor, channel_out=n_competitor, kernel_size=ksize,
                                             stride=1, padding=ksize // 2, init_ratio=init_ratio)
...
class ArcMarginProduct(nn.Module):
    r"""Implement of large margin arc distance:: 实现大边距弧度距离的类
        Args:
